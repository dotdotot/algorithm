...
// 심각한 결함이 발생하는 단점이 있어서, 이전 작동시기에 k광년을 이동하였을 때는 k-1 , k 혹은 k+1 광년만을 다시 이동할 수 있다.
...
int main()
{
    ...
    for (int i = 0; i < testCase; i++)
    {
        ...
        else if (((count * count) < (Y[i] - X[i])) &&
                 ((Y[i] - X[i]) <= (count * (count + 1))))
        {
            cout << 2 * count << endl;
        }
        else if (((count * (count + 1)) < (Y[i] - X[i])) &&
                 ((Y[i] - X[i]) < ((count + 1) * (count + 1))))
        {
            cout << 2 * count + 1 << endl;
        }
    }
